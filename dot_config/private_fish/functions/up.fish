function up --description 'docker compose up hippo, thor-web, event-streaming'
    if test $argv[1] && test $argv[1] = hippo
        ...
        echo "ENABLE_MIRAGE_MOCK=True SKIP_INSTALL=True MINIFY_JS=False SKIP_LEGACY_FE_BUILD=False docker compose -f docker-compose.yml up -d --remove-orphans"
    ...
    if test $argv[1] && test $argv[1] = event
        echo "Docker composing up hippo and event streaming"
        echo "ENABLE_MIRAGE_MOCK=True SKIP_INSTALL=True MINIFY_JS=False SKIP_LEGACY_FE_BUILD=False docker compose -f docker-compose.yml -f docker-compose-event-streaming.yml up -d --remove-orphans"
